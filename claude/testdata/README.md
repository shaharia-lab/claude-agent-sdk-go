...
| `messages/task_lifecycle_completed.jsonl`, `messages/task_lifecycle_killed.jsonl` | `claude` 2.1.224 | Two real background-task lifecycles (#29), captured by asking the CLI to run `Bash` with `run_in_background` and, for the second, to stop it with `TaskStop`. Both are ordered `.jsonl` sequences. They prove the two-vocabulary rule: the stopped task reports **`killed`** via `task_updated` and **`stopped`** via `task_notification`. Crucially, `task_updated` carries its status **inside `patch`**, not at the top level — a consumer reading a top-level `status` sees nothing. (Python warns the notification is *sometimes* suppressed for a killed task; in this capture it did arrive, so the "no notification" path is handled by design but not evidenced here.) |
| `messages/rate_limit_event.json` | `claude` 2.1.224 | A real `rate_limit_event` (#29). The inner `rate_limit_info` is **camelCase** (`resetsAt`, `rateLimitType`, `overageStatus`, `overageDisabledReason`, `isUsingOverage`) while the enclosing message is snake_case — the same split as `modelUsage`. Note `isUsingOverage` appears in no SDK type definition, and neither `utilization` nor `errorCode` was present in this capture. |
| `messages/system_init_full.json` | `claude` 2.1.224 | A complete `system`/`init` (#29) carrying the fields the SDK previously ignored: `capabilities` (the feature-detection list — it is here, **not** in the initialize control response, see #19), `mcp_servers` as `{name,status}` objects, `output_style`, `fast_mode_state`, `uuid`. Also carries `analytics_disabled`, `memory_paths`, `messaging_socket_path`, `product_feedback_disabled` and `fast_mode_disabled_reason`, which remain untyped — read `Event.Raw`. |
